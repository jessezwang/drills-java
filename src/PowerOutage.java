...
public class PowerOutage {
	public static final class FibonacciHeap<T> {
	    ...
	    public static final class Entry<T> {
	         ...
	         */
	        public Entry(T elem, int priority) {
	            mNext = mPrev = this;
	            mElem = elem;
	            mPriority = priority;
	        }
	...
	private void visit(FibonacciHeap.Entry<Integer> v)
    {  // Mark v and add to pq all edges from v to unmarked vertices.
		marked[v.getValue()] = true;
		set.add(v.getValue());
		for (Edge e : G.adj(v.getValue())){
			int w = e.other(v.getValue());
			if (marked[w]) continue;
			if (e.weight() < distTo[w]) {  
				// Edge e is new best connection from tree to w.
				edgeTo[w] = e;
				if (set.contains(w)) {
					pq.decreaseKey(new FibonacciHeap.Entry<Integer>(w, distTo[w]), e.weight());
				} else{
...
//					System.out.println(K);
	...
	public String weight(){
		...
		for (int v=1; v<G.V(); v++)
			if (!marked[v])
				return "Impossible!";		
		PriorityQueue<Integer> redundant = new PriorityQueue<Integer>();
		for (int v=1; v<G.V(); v++){
			redundant.offer(distTo[v]);
		}
		for (int i=1; i<G.V()-K; i++)
			TotalWeight += redundant.poll();
